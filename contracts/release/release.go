...
type Config struct {
	Oracle common.Address // ballstest address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the ballstest service dependency to access the blockchain
	var apiBackend ethapi.Backend
	var ballstest *eth.ballstest
	if err := ctx.Service(&ballstest); err == nil {
		apiBackend = ballstest.ApiBackend
	} else {
		var ballstest *les.Lightballstest
		if err := ctx.Service(&ballstest); err == nil {
			apiBackend = ballstest.ApiBackend
		} else {
			return nil, err
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/ballstest/go-ballstest/releases for new releases")
