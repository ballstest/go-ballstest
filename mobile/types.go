...
// ballstestex retrieves the hex string representation of the block nonce.
func (n *Nonce) ballstestex() string {
...
// ballstestex retrieves the hex string representation of the bloom filter.
func (b *Bloom) ballstestex() string {
	return fmt.Sprintf("0x%x", b.bloom[:])
}

// Header represents a block header in the ballstest blockchain.
type Header struct {
	header *types.Header
}
...
// Block represents an entire block in the ballstest blockchain.
type Block struct {
	block *types.Block
}
...
// Transaction represents a single ballstest transaction.
...
// Deprecated: use ballstestClient.TransactionSender
