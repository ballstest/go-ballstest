...
// Package ballstest contains the simplified mobile APIs to go-ballstest.
//
// The scope of this package is *not* to allow writing a custom ballstest client
// with pieces plucked from go-ballstest, rather to allow writing native dapps on
...
// from the go-ballstest repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-ballstest version.
