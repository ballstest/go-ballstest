...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", ballstest.LightScryptN, ballstest.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.ballstest", new NodeConfig());
			node.start();
			...
			ballstestClient ec = node.getballstestClient();
	...
	// signatures too. Regression test for go-ballstest issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altoballstester
	...
	workspace, err := ioutil.TempDir("", "ballstest-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/ballstest/ballstesttest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for ballstest and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ballstest", "github.com/ballstest/go-ballstest/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "ballstest.aar"), "ballstest.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "ballstest", "ballstesttest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.ballstest.ballstesttest"
...
dependencies {
    ...
    compile(name: "ballstest", ext: "aar")
}
`
