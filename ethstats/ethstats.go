...
// Service implements an ballstest netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	server *p2p.Server        // Peer-to-peer server to retrieve networking infos
	eth    *eth.ballstest      // Full ballstest service if monitoring a full node
	les    *les.Lightballstest // Light ballstest service if monitoring a light node
...
func New(url string, ethServ *eth.ballstest, lesServ *les.Lightballstest) (*Service, error) {
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	for i, number := range indexes {
		...
		if s.eth != nil {
			...
			if header := s.les.BlockChain().ballstesteaderByNumber(number); header != nil {
