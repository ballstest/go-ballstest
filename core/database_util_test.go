...
import (
	...
	"github.com/ballstest/go-ballstest/crypto/sha3"
...
func TestHeaderStorage(t *testing.T) {
	...
	if entry := ballstesteader(db, header.Hash(), header.Number.Uint64()); entry != nil {
	...
	if entry := ballstesteaderRLP(db, header.Hash(), header.Number.Uint64()); entry == nil {
...
func TestBlockStorage(t *testing.T) {
	...
	if entry := ballstesteader(db, block.Hash(), block.NumberU64()); entry != nil {
		t.Fatalf("Non existent header returned: %v", entry)
	}
...
func TestHeadStorage(t *testing.T) {
	...
	if entry := ballstesteadHeaderHash(db); entry != (common.Hash{}) {
		t.Fatalf("Non head header entry returned: %v", entry)
	}
	if entry := ballstesteadBlockHash(db); entry != (common.Hash{}) {
		t.Fatalf("Non head block entry returned: %v", entry)
	}
	if entry := ballstesteadFastBlockHash(db); entry != (common.Hash{}) {
	...
	if entry := ballstesteadHeaderHash(db); entry != blockHead.Hash() {
		t.Fatalf("Head header hash mismatch: have %v, want %v", entry, blockHead.Hash())
	}
	if entry := ballstesteadBlockHash(db); entry != blockFull.Hash() {
		t.Fatalf("Head block hash mismatch: have %v, want %v", entry, blockFull.Hash())
	}
	if entry := ballstesteadFastBlockHash(db); entry != blockFast.Hash() {
