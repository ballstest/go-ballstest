...
// Package les implements the Light ballstest Subprotocol.
package les
...
import (
	...
	"github.com/ballstest/go-ballstest/core/state"
	...
	"github.com/ballstest/go-ballstest/light"
	"github.com/ballstest/go-ballstest/log"
	"github.com/ballstest/go-ballstest/p2p"
	"github.com/ballstest/go-ballstest/p2p/discover"
	"github.com/ballstest/go-ballstest/p2p/discv5"
	"github.com/ballstest/go-ballstest/params"
	"github.com/ballstest/go-ballstest/rlp"
	"github.com/ballstest/go-ballstest/trie"
)
...
type BlockChain interface {
	...
	ballstesteader(hash common.Hash, number uint64) *types.Header
	ballstesteaderByHash(hash common.Hash) *types.Header
	CurrentHeader() *types.Header
	GetTdByHash(hash common.Hash) *big.Int
	InsertHeaderChain(chain []*types.Header, checkFreq int) (int, error)
	Rollback(chain []common.Hash)
	Status() (td *big.Int, currentBlock common.Hash, genesisBlock common.Hash)
	ballstesteaderByNumber(number uint64) *types.Header
...
// NewProtocolManager returns a new ballstest sub protocol manager. The ballstest sub protocol manages peers capable
// with the ballstest network.
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light ballstest protocol")
	...
	log.Info("Light ballstest protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light ballstest peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light ballstest message handling failed", "err", err)
			return err
		}
	}
}

var reqList = []uint64{GetBlockHeadersMsg, GetBlockBodiesMsg, GetCodeMsg, GetReceiptsMsg, GetProofsV1Msg, SendTxMsg, SendTxV2Msg, GetTxStatusMsg, ballstesteaderProofsMsg, GetProofsV2Msg, ballstestelperTrieProofsMsg}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light ballstest message arrived", "code", msg.Code, "bytes", msg.Size)
	...
	case GetBlockHeadersMsg:
		...
		for !unknown && len(headers) < int(query.Amount) && bytes < softResponseLimit {
			...
			if hashMode {
				origin = pm.blockchain.ballstesteaderByHash(query.Origin.Hash)
			} else {
				origin = pm.blockchain.ballstesteaderByNumber(query.Origin.Number)
			}
			if origin == nil {
				break
			}
			...
			case query.Origin.Hash != (common.Hash{}) && query.Reverse:
				...
				for i := 0; i < int(query.Skip)+1; i++ {
					if header := pm.blockchain.ballstesteader(query.Origin.Hash, number); header != nil {
			...
			case query.Origin.Hash != (common.Hash{}) && !query.Reverse:
				...
				if header := pm.blockchain.ballstesteaderByNumber(origin.Number.Uint64() + query.Skip + 1); header != nil {
	...
	case GetCodeMsg:
		...
		for _, req := range req.Reqs {
			...
			if header := core.ballstesteader(pm.chainDb, req.BHash, core.GetBlockNumber(pm.chainDb, req.BHash)); header != nil {
	...
	case GetReceiptsMsg:
		...
		for _, hash := range req.Hashes {
			...
			if results == nil {
				if header := pm.blockchain.ballstesteaderByHash(hash); header == nil || header.ReceiptHash != types.EmptyRootHash {
					continue
				}
			}
	...
	case ballstesteaderProofsMsg:
		...
		for _, req := range req.Reqs {
			...
			if header := pm.blockchain.ballstesteaderByNumber(req.BlockNum); header != nil {
	...
	case ballstestelperTrieProofsMsg:
		...
		for _, req := range req.Reqs {
			...
			if tr == nil || req.HelperTrieType != lastType || req.TrieIdx != lastIdx {
				var prefix string
				root, prefix = pm.ballstestelperTrie(req.HelperTrieType, req.TrieIdx)
				if root != (common.Hash{}) {
			...
			if req.AuxReq == auxRoot {
				...
				if req.AuxReq != 0 {
					data := pm.ballstestelperTrieAuxData(req)
					auxData = append(auxData, data)
					auxBytes += len(data)
				}
			}
...
// ballstestelperTrie returns the post-processed trie root for the given trie ID and section index
func (pm *ProtocolManager) ballstestelperTrie(id uint, idx uint64) (common.Hash, string) {
	switch id {
	case htCanonical:
...
// ballstestelperTrieAuxData returns requested auxiliary data for the given HelperTrie request
func (pm *ProtocolManager) ballstestelperTrieAuxData(req HelperTrieReq) []byte {
	if req.HelperTrieType == htCanonical && req.AuxReq == auxHeader {
		...
		return core.ballstesteaderRLP(pm.chainDb, hash, blockNum)
	}
	return nil
}
...
// NodeInfo represents a short summary of the ballstest sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // ballstest network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
