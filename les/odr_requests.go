...
// for the ballstest Light Client.
package les
...
func (r *BlockRequest) Validate(db ethdb.Database, msg *Msg) error {
	...
	header := core.ballstesteader(db, r.Hash, r.Number)
	if header == nil {
...
func (r *ChtRequest) GetCost(peer *peer) uint64 {
	switch peer.version {
	case lpv1:
		return peer.GetRequestCost(ballstesteaderProofsMsg, 1)
	case lpv2:
		return peer.GetRequestCost(ballstestelperTrieProofsMsg, 1)
	default:
		panic(nil)
	}
}
...
func (r *BloomRequest) GetCost(peer *peer) uint64 {
	return peer.GetRequestCost(ballstestelperTrieProofsMsg, len(r.SectionIdxList))
}
