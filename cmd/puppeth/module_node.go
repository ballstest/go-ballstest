...
// nodeDockerfile is the Dockerfile required to run an ballstest node.
var nodeDockerfile = `
FROM ballstest/client-go:latest
...
{{end}}
RUN \
  echo 'ballstest --cache 512 init /genesis.json' > ballstest.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.ballstest/keystore/ && cp /signer.json /root/.ballstest/keystore/' >> ballstest.sh && \{{end}}
	echo $'ballstest --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .BootV4}}--bootnodesv4 {{.BootV4}}{{end}} {{if .BootV5}}--bootnodesv5 {{.BootV5}}{{end}} {{if .Etherbase}}--etherbase {{.Etherbase}} --mine --minerthreads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> ballstest.sh

ENTRYPOINT ["/bin/sh", "ballstest.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an ballstest node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    volumes:
      - {{.Datadir}}:/root/.ballstest{{if .Ethashdir}}
...
// deployNode deploys a new ballstest node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 ballstest --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.ballstest"],
