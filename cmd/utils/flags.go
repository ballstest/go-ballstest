...
// Package utils contains internal helper functions for go-ballstest commands.
package utils
...
import (
	...
	"github.com/ballstest/go-ballstest/dashboard"
	...
	"github.com/ballstest/go-ballstest/p2p/netutil"
...
// for ballstest and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `ballstest account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an ballstest client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the ballstest service: %v", err)
	}
}
...
// RegisterEthStatsService configures the ballstest Stats daemon and adds it to
// th egiven node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both eth and les services
		var ethServ *eth.ballstest
		ctx.Service(&ethServ)
		...
		Fatalf("Failed to register the ballstest Stats service: %v", err)
	}
}
...
// new format.
//
// e.g. ballstest account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// ballstest --keystore /tmp/mykeystore --lightkdf account new
//
