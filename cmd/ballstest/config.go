...
import (
	...
	"github.com/ballstest/go-ballstest/contracts/release"
...
func loadConfig(file string, cfg *ballstestConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "ballstest.ipc"
	return cfg
}

func makeConfigNode(ctx *cli.Context) (*node.Node, ballstestConfig) {
	// Load defaults.
	cfg := ballstestConfig{
...
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	// Add the ballstest Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
	...
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		utils.Fatalf("Failed to register the ballstest release oracle service: %v", err)
	}
	return stack
}
