...
import (
	...
	"github.com/ballstest/go-ballstest/console"
	"github.com/ballstest/go-ballstest/contracts/ens"
	"github.com/ballstest/go-ballstest/crypto"
	"github.com/ballstest/go-ballstest/ethclient"
	"github.com/ballstest/go-ballstest/internal/debug"
	...
	"github.com/ballstest/go-ballstest/swarm"
	bzzapi "github.com/ballstest/go-ballstest/swarm/api"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the ballstest API provider to use to settle SWAP payments",
	...
	EnsAPIFlag = cli.StringFlag{
		Name:   "ens-api",
		Usage:  "URL of the ballstest API provider to use for ENS record lookups",
...
// This init function sets defaults so cmd/swarm can run alongside ballstest.
func init() {
...
var app = utils.NewApp(gitCommit, "ballstest Swarm")

// This init function creates the cli.App.
func init() {
	app.Action = bzzd
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2016 The go-ballstest Authors"
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.ballstest/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.ballstest/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.ballstest/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.ballstest/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	...
	//ballstest only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for ballstest and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the ballstest node
	...
	//register BZZ as node.Service in the ballstest node
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	//register within the ballstest node
