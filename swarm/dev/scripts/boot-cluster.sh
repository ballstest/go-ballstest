...
#
# The cluster contains a bootnode, a ballstest node and multiple swarm nodes, with
...
# static ballstest configuration
ballstest_IP="192.168.33.3"
ballstest_RPC_PORT="8545"
ballstest_RPC_URL="http://${ballstest_IP}:${ballstest_RPC_PORT}"
...
# network namespaces toballstester
create_network() {
...
# start_bootnode starts a bootnode which is used to bootstrap the ballstest and
# swarm nodes
start_bootnode() {
...
# start_ballstest_node starts a ballstest node with --datadir pointing at <base-dir>/ballstest
# and a single, unlocked account with password "ballstest"
start_ballstest_node() {
  local dir="${base_dir}/ballstest"
  mkdir -p "${dir}"

  local password="ballstest"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating ballstest account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get ballstest account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${ballstest_IP}"
    --rpcport   "${ballstest_RPC_PORT}"
    --verbosity "6"
  )

  start_node "ballstest" "${ballstest_IP}" "$(which ballstest)" ${args[@]}
}
...
create_account() {
  ...
  ballstest --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
