...
// ecrecover extracts the ballstest account address from a signed header.
func ecrecover(header *types.Header, sigcache *lru.ARCCache) (common.Address, error) {
	...
	// Recover the public key and the ballstest address
...
// ballstest testnet following the Ropsten attacks.
type Clique struct {
	...
	signer common.Address // ballstest address of the signing key
...
// Author implements consensus.Engine, returning the ballstest address recovered
...
func (c *Clique) snapshot(chain consensus.ChainReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
	...
	for snap == nil {
		...
		if number == 0 {
			genesis := chain.ballstesteaderByNumber(0)
		...
		if len(parents) > 0 {
			...
			header = chain.ballstesteader(hash, number)
			if header == nil {
				return nil, consensus.ErrUnknownAncestor
			}
		}
