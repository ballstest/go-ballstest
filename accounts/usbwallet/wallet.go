...
import (
	...
	ballstest "github.com/ballstest/go-ballstest"
...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the ballstest
...
type wallet struct {
	...
	deriveChain    ballstest.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived ballstest account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain ballstest.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
